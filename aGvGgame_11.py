...
def intro():
    ...
    winm = pygame.display.set_mode((600,480))
    pygame.display.set_caption("DRAGON BALL")
    menubg = pygame.image.load('gvgmenu1.png')
    htpimg = pygame.image.load('HTPwind.png')

    StartP= pygame.image.load('startPressed.png')
    StartUP = pygame.image.load('startUnpressed.png')
    QuitP = pygame.image.load('quitPressed.png')
    QuitUP = pygame.image.load('quitUnpressed.png')
    HtpP= pygame.image.load('HTPPressed.png')
    HtpUP= pygame.image.load('HTPUnpressed.png')
    ...
    ##        self.text = text
        ...
        def draw(self,winm,butt=None):
                if self.press and butt ==  "start":
                    winm.blit(StartP, (self.x,self.y))
                elif butt == "start":
                    winm.blit(StartUP, (self.x,self.y))
                elif self.press and butt ==  "htp":
                    winm.blit(HtpP, (self.x,self.y))
                elif butt == "htp":
                    winm.blit(HtpUP, (self.x,self.y))
                elif self.press and butt ==  "quit":
                    winm.blit(QuitP, (self.x,self.y))
                elif butt == "quit":
                    winm.blit(QuitUP, (self.x,self.y))
        ...
        def isOver(self, pos):
            ...
            return False
    def redrawWindow():
        winm.blit(menubg,(0,0))
        StartButton.draw(winm,"start")
        HtpButton.draw(winm,"htp")
        QuitButton.draw(winm,"quit")
        if htppress:
            winm.blit(htpimg, (10,100))

    run = True
    StartButton = button(305,250)
    HtpButton = button(305,298)
    QuitButton = button(305,348)
    htppress= False
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                ...
                if HtpButton.isOver(pos):
                    HtpButton.press = True
                else:
                    HtpButton.press = False
                    
                if QuitButton.isOver(pos):
                    QuitButton.press = True
                else:
                    QuitButton.press = False

            keysm = pygame.key.get_pressed()
...
def gameLoop():
    ...
    bg = pygame.image.load('bg1111.png') #bg image
